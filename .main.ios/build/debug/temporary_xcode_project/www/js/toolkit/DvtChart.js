...
DvtPieChart.prototype.render = function() {
  ...
  // Bug 12958165: Don't render shadows in Chrome SVG
  if (!DvtAgent.isPlatformWebkit()) {
    //BUG 12427741: apply shadow after rendering slices because
...
DvtChart.prototype.render = function(options, width, height) 
{
  ...
  // Bug 21148456. We don't want the inner chart's listeners to be invoked when it is a spark chart, thus we are removing them.
...
DvtChartObjPeer.prototype.isDoubleClickable = function() {
  // Bug 22322561: IE double clicking workaround in DvtEventManager.
...
DvtChartObjPeer.prototype.getNextNavigable = function(event) {
  ...
  // Polar bars should be treated the same way as line/area charts
    ...
    // Bug 21091620: ignoreBounds for the case of range bars or bars with negative values that don't overlap with the adjacent series.
  ...
  if (DvtChartTypeUtils.isStacked(chart)) {
    ...
    for (var i = 0; i <= seriesIndex; i++) {
      ...
      if (DvtChartDataUtils.getStackCategory(chart, i) == category) {
        ...
        // Bug 21078095, only add up positive values of items if current bar being processed is positive, and vice versa.
...
DvtPieRenderUtils.createTopSurface = function(slice, fill) {
  ...
  if (!slice.getStrokeColor() && pieChart.getSkin() == DvtCSSStyle.SKIN_SKYROS && pieChart.is3D() && slice.getDepth() > 0 && DvtChartStyleUtils.getSeriesEffect(pieChart.chart) == 'gradient' &&
      (slice.getAngleStart() >= 180 || (slice.getAngleStart() + slice.getAngleExtent()) >= 180 || slice.getAngleExtent() == 360))// Bug 12419047; slice.getAngleExtent() == 360 means we only have one slice
  {
  ...
  if (DvtChartEventUtils.isScrollable(chart) && DvtChartTypeUtils.isHorizScrollbarSupported(chart)) {
    ...
    if (DvtChartStyleUtils.isOverviewRendered(chart)) {
      ...
      if (height > 0) {
        ...
        DvtLayoutUtils.position(availSpace, 'bottom', chart.overview, width, height, 10); // TODO #omitpanho#endomit: store as default
      }
    }
...
DvtChartRenderer._adjustAvailSpace = function(availSpace) {
  // Bug 16446255: Adjust the bounds to the closest pixel to prevent antialiasing issues.
...
DvtPlotAreaRenderer._renderForegroundObjects = function(chart, container, availSpace) {
  // Bug 19475329: Axis lines are generally rendered in the back of the foreground, but we render them after the
...
DvtPlotAreaRenderer.createClippedGroup = function(chart, container, availSpace) {
  ...
  // Bug 20393781 - line gets cut off at plot area max
